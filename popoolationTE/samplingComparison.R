...
# create function to count number of TEs (all, not just FR)
countTEsAll <- function(filename){
  dat <- read.delim(filename, header=FALSE)
  colnames(dat) <- as.matrix(header)
  dat <- merge(hierarchy, (dat %>% filter(ID %in% chrom) %>% 
                             filter(refID =="-") %>%
...
header <- read.table("~/GitHub/flyPopGenomics/popoolationTE/te-polyfiltered.header.txt", header=FALSE)
...
# aggregate popoolationTE counts for all populations (all results)
countTable <- hierarchy
for (name in files){
  print(name)
  countTable <- merge(countTable, countTEsAll(name), by.x=c("family", "type"), by.y=c("family", "type"))
}
colnames(countTable) <- c("family","type","ACO1","ACO2","ACO3","ACO4","ACO5","AO1","AO2","AO3","AO4","AO5","B1","B2","B3","B4","B5","BO1","BO2","BO3","BO4","BO5","CO1","CO2","CO3","CO4","CO5","nCO1","nCO2","nCO3","nCO4","nCO5")
popSums <- apply(countTable[,3:32], 2, sum, na.rm=TRUE)

# statistical comparisons for popoolationTE (all results)
ACO <- popSums[1:5] 
AO <- popSums[6:10]
B <- popSums[11:15]
BO <- popSums[16:20]
