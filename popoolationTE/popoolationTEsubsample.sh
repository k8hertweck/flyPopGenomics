...
cd /scratch/03177/hertweck/fly/"$1"sub
					...
					then
						...
						if [ -f $1"$x"R1pop.fastq ]
							then 
								...
								echo "converting R1"
								zcat ../$1/$1"$x"R1.fastq.gz | head -80000000 | awk '{if ($1 ~ /SRR/) print $0 "/1"; else print $0}' | tr -d " " > $1"$x"R1pop.fastq
						fi
						if [ -f $1"$x"R2pop.fastq ]
							then 
								...
								echo "converting R2"
								zcat ../$1/$1"$x"R2.fastq.gz | head -80000000 | awk '{if ($1 ~ /SRR/) print $0 "/2"; else print $0}' | tr -d " " > $1"$x"R2pop.fastq
						fi				
